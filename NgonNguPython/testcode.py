...
class Ship:
    def __init__(self, size_name, positions: list):
        self.size_name = size_name # 2, 3, 4, 5 hoặc "2x2"
...
running = True
while running:
    if has_bg: screen.blit(bg_image, (0, 0))
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                ...
                if GRID_OFFSET_X_HUMAN <= mx < GRID_OFFSET_X_HUMAN + GRID_SIZE_PX and \
                   GRID_OFFSET_Y <= my < GRID_OFFSET_Y + GRID_SIZE_PX:
                    ...
                    if size == "2x2":
                        if gx + 1 < 10 and gy + 1 < 10: positions = [(gx, gy), (gx + 1, gy), (gx, gy + 1), (gx + 1, gy + 1)]
                        else: is_valid = False
                    else:
                        if placement_horizontal:
                            if gx + size <= 10: positions = [(gx + i, gy) for i in range(size)]
                            else: is_valid = False
                        else:
                            if gy + size <= 10: positions = [(gx, gy + i) for i in range(size)]
                            else: is_valid = False
                    ...
                    if is_valid:
                        board_human.add_ship(Ship(size, positions))
                        placement_index += 1
                        if placement_index >= len(placement_sizes): set_state('PLAYING')
    ...
    if current_state == 'PLAYING' and current_turn == "AI" and not game_over:
        ...
        else: current_turn = "HUMAN"
            
    if current_state == 'PLAYING': explosion_manager.update()

    if current_state == 'MAIN': draw_main_menu()
    elif current_state == 'DIFFICULTY': draw_difficulty_menu()
    elif current_state == 'SETTINGS': draw_settings_menu()
    elif current_state == 'HISTORY': draw_history_menu()
    elif current_state == 'PLACEMENT': draw_placement_state()
    elif current_state == 'PLAYING': draw_playing_state()
