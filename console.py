...
def load_match_history():
    if not os.path.exists(HISTORY_FILE):
        ...
        try:
            ...
            print(f"Console Log - Error creating history file: {e}")
        return default_data
    ...
    try:
        ...
        print(f"Console Log - Error reading history file: {e}")
...
def save_match_history(winner):
    ...
    try:
        ...
        print(f"Console Log - Match Saved: {winner} won match #{data['total_matches']}")
    except Exception as e:
        print(f"Console Log - Error saving history file: {e}")
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_t: 
                ai_ships_left = 0
                print("Console Log - Cheat activated: Faking Player Victory")
